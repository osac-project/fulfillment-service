...
var _ = Describe("Private bare metal instance types server", func() {
	...
	Describe("Behaviour", func() {
		...
		Describe("Input validation", func() {
			It("Rejects create request with missing object", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{}.Build())
				Expect(err).To(HaveOccurred())
				status, ok := grpcstatus.FromError(err)
				Expect(ok).To(BeTrue())
			...
			It("Rejects create request with missing hardware", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.hardware' is mandatory"))
			})

			It("Rejects create request with missing CPU spec", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
							Name: "missing-cpu",
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.hardware.cpu' is mandatory"))
			})

			It("Rejects create request with invalid CPU cores", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
							Name: "invalid-cores",
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.hardware.cpu.cores' must be greater than zero"))
			})

			It("Rejects create request with missing CPU architecture", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
							Name: "missing-arch",
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.hardware.cpu.architecture' is mandatory"))
			})

			It("Rejects create request with invalid threads per core", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.hardware.cpu.threads_per_core' must be greater than zero"))
			})

			It("Rejects create request with missing memory spec", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
							Name: "missing-memory",
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.hardware.memory' is mandatory"))
			})

			It("Rejects create request with invalid memory total", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
							Name: "invalid-memory",
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.hardware.memory.total_gb' must be greater than zero"))
			})

			It("Rejects create request with invalid disk type", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
							Name: "invalid-disk",
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.hardware.disks[0].type' is mandatory"))
			})

			It("Rejects create request with missing host label selector", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
							Name: "missing-labels",
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.host_label_selector' is mandatory"))
			})

			It("Rejects create request with empty match labels", func() {
				_, err := server.Create(ctx, privatev1.BareMetalInstanceTypesCreateRequest_builder{
					Object: privatev1.BareMetalInstanceType_builder{
						Metadata: privatev1.Metadata_builder{
							Name: "empty-labels",
				...
				Expect(ok).To(BeTrue())
				Expect(status.Code()).To(Equal(grpccodes.InvalidArgument))
				Expect(status.Message()).To(Equal("field 'spec.host_label_selector.match_labels' must contain at least one label pair"))
			})
		})
