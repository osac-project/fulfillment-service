...
type JwksCache interface {
	...
	// posible to load it from one of the configured issuers, an error is returned.
	//
...
func (i *jwksCache) Get(ctx context.Context, issuerUrl string, keyId string) (result any, err error) {
	...
	keyObj, ok := i.getEntry(issuerUrl, keyId)
	if ok {
		if i.maxTTL > 0 && issuerInfo.age() > i.maxTTL {
			refreshCtx := context.WithoutCancel(ctx)
			go func() {
				err := i.refresh(refreshCtx, issuerInfo)
				if err != nil {
...
// is false.
func (i *jwksCache) getEntry(issuerUrl string, keyId string) (result any, ok bool) {
	i.cacheLock.RLock()
...
func (i *jwksCache) replaceEntries(issuerUrl string, keyMap map[string]any) {
	i.cacheLock.Lock()
	defer i.cacheLock.Unlock()
	for keyTag := range i.cache {
		if keyTag.issuerUrl == issuerUrl {
			delete(i.cache, keyTag)
		}
	}
...
func (i *jwksCache) refresh(ctx context.Context, issuerInfo *jwksCacheIssuerInfo) error {
	issuerInfo.refreshLock.Lock()
	defer issuerInfo.refreshLock.Unlock()
	if issuerInfo.age() <= i.minTTL {
		return nil
	}
	...
	i.replaceEntries(issuerInfo.issuerUrl, keyMap)
...
func (i *jwksCache) loadKeys(ctx context.Context, issuerInfo *jwksCacheIssuerInfo) (result map[string]any,
	err error) {
...
func (i *jwksCache) discoverJwksUrl(ctx context.Context, issuerInfo *jwksCacheIssuerInfo) error {
...
func (i *jwksCache) loadJwksUrl(ctx context.Context, issuerInfo *jwksCacheIssuerInfo) (result map[string]any,
	err error) {
...
func (i *jwksCache) loadTokenFile(tokenFile string) (result string, err error) {
	tokenBytes, err := os.ReadFile(tokenFile) //nolint:gosec
	if err != nil {
...
func (i *jwksCache) readKeys(ctx context.Context, issuerInfo *jwksCacheIssuerInfo,
...
func (i *jwksCache) parseKey(data jwksCacheKeyData) (result any, err error) {
	if data.Kty == "" {
