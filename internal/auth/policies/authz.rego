...
#
# TODO: The 'template-publisher' and 'osac-controller-manager' service accounts are used by the template publisher and
# by the OSAC operator. That assumes that they are in the same Kubernetes clusters, which is wrong, but we need it for
# now. They should be replaced with Keycloak service accounts.
emergency_service_account_names := {
  "admin",
  "template-publisher",
  "osac-controller-manager",
}
...
# Subject construction (replaces Authorino CEL response mapping):
